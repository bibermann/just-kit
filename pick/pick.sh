...
auto_generated_hint_lines=(
  "# Note: The imports above were generated by \`just $recipe_running_this_script\`"
  '# Docs: https://github.com/bibermann/just-kit?tab=readme-ov-file#just-pick'
  ''
  )

printf -v auto_generated_hint "%s\n" "${auto_generated_hint_lines[@]}"
...
echo "$auto_generated_hint" >>"$temp_file"

skip_next_empty_line=0
while IFS= read -r line; do
  if [[ "$line" == "${auto_generated_hint_lines[0]}" ]]; then
    skip_next_empty_line=$((${#auto_generated_hint_lines[@]} - 1))
    continue
  fi
  if [[ $skip_next_empty_line -gt 0 ]]; then
    next_hint_line="${auto_generated_hint_lines[$((${#auto_generated_hint_lines[@]} - skip_next_empty_line))]}"
    if [[ "$line" == "$next_hint_line" ]]; then
      skip_next_empty_line=$((skip_next_empty_line - 1))
      continue
    fi
  fi
  skip_next_empty_line=0
