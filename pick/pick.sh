...
function print_clean_path() {
    ...
    # Extract the filename without .just extension
    local name=$(basename "$path" .just)

    # Get the directory part
    local dirname=$(dirname "$path")

    # Split the path into components and filter out . .. and .just
    local parts=()
    local IFS='/'
...
function get_justfile_recipes() {
  ...
  while true; do
    ...
    if output="$(JUST_UNSTABLE=1 just --allow-missing --dump --dump-format json -f "$temp_file" 2>&1)"; then
      # Check if the $recipe_checking_relevance recipe exists and run it if it does
      if grep -q "^$recipe_checking_relevance:" "$full_path" 2>/dev/null; then
        # Use just to run the $recipe_checking_relevance recipe
...
# Process each item in the options array
if [ "$PWD" = "/" ] || [ "$PWD" = "/home" ] || echo "$PWD" | grep -q "^/home/[^/]*$"; then
  current_dir_is_to_high=1
else
  current_dir_is_to_high=0
fi
...
for path in "${paths[@]}"; do
  ...
  if [[ -v name_counts[$name] ]]; then
    ...
    # Append count to name to make it unique
    unique_name="${name} ${name_counts[$name]}"
  else
    # First occurrence - no need to change the name
    name_counts[$name]=1
    unique_name="$name"
  fi
...
for ((i = 0; i < ${#path_list[@]}; i++)); do
  ...
  for ((j = i + 1; j < ${#path_list[@]}; j++)); do
    ...
    # If there are shared recipes, add to conflicts
...
for conflict in "${!conflicts[@]}"; do
  ...
  shared_recipes="${conflicts[$conflict]}"

  # Check if either path exists in overrides_array
  ...
  echo "1. $path1"
  echo "2. $path2"
  echo "Shared recipes: $shared_recipes"
...
# Store original path order
original_order=("${paths[@]}")

topological_sort() {
  local node="$1"

  # Return if node is already in sorted_paths (permanent mark)
  ...
  # Mark node permanently and add to sorted_paths
  visited[$node]=1
  sorted_paths+=("$node")
}
...
echo -e >>"$temp_file" "$auto_generated_hint\n"
